...
export class MapsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.map.on('load', () => {
      ...
      // Add a layer to show circle points
      this.map.addLayer({
        id: 'circle_layer',
        type: 'circle',
        source: 'points',
      ...
      this.map.on('mousemove', 'circle_layer', (e: any) => {
        ...
        const finalRes = res.filter(
          (item: any) => item?.properties?.id || false
        );
        this.map.setFilter('circle_layer_hover', [
          '==',
          'id',
          finalRes[0]?.properties?.id,
        ]);
      ...
      //add a layer to display the place name
      this.map.addLayer({
        id: 'points',
        type: 'symbol',
        source: 'points',
      ...
      // add a layer to show line between points
      this.map.addLayer({
        id: 'line',
        type: 'line',
        source: 'points',
    ...
    this.map.on('load', () => {
      this.map.loadImage(
        ...
        (error, image2) => {
          ...
          // adda a layer to show aeroplane icon 
          this.map.addLayer({
            id: 'aeroplane-layer',
            type: 'symbol',
            source: 'path',
